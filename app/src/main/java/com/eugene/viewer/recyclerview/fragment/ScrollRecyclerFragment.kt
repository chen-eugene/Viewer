...
import com.eugene.viewer.adapter.GirlAdapter
 ...
 */
class ScrollRecyclerFragment : BaseFragment() {

    companion object {
        fun newInstance(context: Context, bundle: Bundle?): Fragment {
            return Fragment.instantiate(context, ScrollRecyclerFragment::class.java.name, bundle)
        }
    }

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        return inflater.inflate(R.layout.fragment_scroll_recycler, container, false)
    }

    override fun onActivityCreated(savedInstanceState: Bundle?) {
        super.onActivityCreated(savedInstanceState)
        val data = initData()
        recycler?.adapter = GirlAdapter(activity!!, data)
        recycler?.layoutManager = object : LinearLayoutManager(activity, LinearLayoutManager.VERTICAL, false) {

            /**
             * 外部拦截，禁止RecyclerView竖直方向的滑动
             */
